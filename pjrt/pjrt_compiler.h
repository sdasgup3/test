...
class PjRtPhaseCompiler : public PjRtCompiler {
 public:
  ...
  virtual absl::Status RegisterPhase(const std::string& phase_name,
                                     PhaseCompiler phase_compiler) = 0;
  virtual absl::StatusOr<PhaseCompiler> GetPhaseCompiler(
      const std::string& phase_name) = 0;
  virtual absl::StatusOr<std::vector<std::string>> GetPhaseNames() = 0;
  ...
  absl::StatusOr<std::vector<PjRtPartialProgram>> CompilePhase(
    ...
    // Compile the programs in the order of phases_to_run.
