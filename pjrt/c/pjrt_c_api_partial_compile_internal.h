...
#ifndef TENSORFLOW_COMPILER_XLA_PJRT_C_PJRT_C_API_PARTIAL_COMPILE_INTERNAL_H_
#define TENSORFLOW_COMPILER_XLA_PJRT_C_PJRT_C_API_PARTIAL_COMPILE_INTERNAL_H_
...
// Returns the names of all the partial compile phases registered with the
// extension.
absl::StatusOr<std::vector<std::string>> GetPhaseNames(
    const PJRT_PartialCompile_Extension* partial_compile_extension);

// Runs the partial compile phase with the given name and returns the compiled
// serialzied programs.
...
// Creates and initializes a PJRT_PartialCompile_Extension struct.
PJRT_PartialCompile_Extension CreatePartialCompileExtension(
    PJRT_Extension_Base* next);

}  // namespace pjrt

#endif  // TENSORFLOW_COMPILER_XLA_PJRT_C_PJRT_C_API_PARTIAL_COMPILE_INTERNAL_H_
